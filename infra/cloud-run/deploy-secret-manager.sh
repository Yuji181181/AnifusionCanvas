...
REPLICATE_MODE="${REPLICATE_MODE:-demo}"
REPLICATE_TOONCRAFTER_VERSION="${REPLICATE_TOONCRAFTER_VERSION:-}"
REPLICATE_SDXL_INPAINTING_VERSION="${REPLICATE_SDXL_INPAINTING_VERSION:-}"
...
ENV_VARS="APP_ENV=production,API_PORT=8080,FRONTEND_ORIGIN=https://anifusion-canvas.pages.dev,R2_REGION=auto,R2_PUBLIC_BASE_URL=${R2_PUBLIC_BASE_URL},STUDIO_STORE=database,REPLICATE_MODE=${REPLICATE_MODE}"
if [[ -n "$REPLICATE_TOONCRAFTER_VERSION" ]]; then
  ENV_VARS="${ENV_VARS},REPLICATE_TOONCRAFTER_VERSION=${REPLICATE_TOONCRAFTER_VERSION}"
fi
if [[ -n "$REPLICATE_SDXL_INPAINTING_VERSION" ]]; then
  ENV_VARS="${ENV_VARS},REPLICATE_SDXL_INPAINTING_VERSION=${REPLICATE_SDXL_INPAINTING_VERSION}"
fi
